...
//Если обработчик событи нужно навесить на несколько кнопок, 
//то используем перебо(forEach)
btns.forEach(btn => {
    btn.addEventListener('click', deleteElement, { once: true });
}); //теперь все кнопки реагируют на клик мыши

//Опции события(в примере выше в обработчик события передаем третий аргумент для выполнения 
//обработки события только один раз({once: true})) после 
//клика мышки